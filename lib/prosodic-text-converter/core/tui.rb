  ...
  class TUI
    ...
    def setup_environment_if_needed
      ...
      unless env_file.exist?
        if @prompt.yes?("No .env file found. Would you like to set up API keys?", color: :yellow)
          setup_environment_variables
        end
      else
        if @prompt.yes?("Update existing .env configuration?", default: false)
          setup_environment_variables
        end
      end
    end
    ...
    def collect_user_options
      ...
      if @prompt.yes?("Generate speech with ElevenLabs?", default: false)
        if ENV['ELEVENLABS_API_KEY']
          ...
          options[:elevenlabs_voice] = @prompt.select("ElevenLabs voice:", voices)
    ...
    def setup_output_directory
      ...
      unless output_path.exist?
        if @prompt.yes?("Directory '#{output_dir}' doesn't exist. Create it?", color: :yellow)
          begin
    ...
    def execute_with_options(options)
      ...
      begin
        ...
        if model.nil? || model.empty?
          raise ArgumentError, "Model is required"
        end
        
        if input_text.nil? || input_text.strip.empty?
          raise ArgumentError, "Input text is required"
        end
        ...
        if config.get(:pitch_backend)
          converter_options[:pitch_backend] = config.get(:pitch_backend).to_sym
        end
        ...
        # Save output
        output_file = File.join(options[:output_dir], "converted_#{Time.now.strftime('%Y%m%d_%H%M%S')}.#{options[:output_format] || 'ssml'}")
        ...
        if options[:elevenlabs_voice] && result[:ssml_output]
          synthesize_speech(result[:ssml_output], options)
        end
    ...
    def build_cli_args(options)
      ...
      args << "--elevenlabs-voice=#{options[:elevenlabs_voice]}" if options[:elevenlabs_voice]
