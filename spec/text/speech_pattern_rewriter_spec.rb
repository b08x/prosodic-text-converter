...
RSpec.describe ProsodicTextConverter::SpeechPatternRewriter do
  ...
  let(:mock_llm_response) do
    double('LLMResponse', content: 'Hello there, world. This is an improved test sentence for speech pattern rewriting.')
  end
  ...
  describe '#initialize' do
    ...
    it 'raises error on initialization failure' do
      ...
      expect { described_class.new(provider: provider, model: model) }.to raise_error(/Failed to initialize SpeechPatternRewriter/)
    end
  end
  ...
  describe '#rewrite_text' do
    ...
    it 'validates input parameters' do
      expect { rewriter.rewrite_text(nil, mock_rewrite_guidance) }.to raise_error(ArgumentError, /Text analysis must be a hash/)
      expect { rewriter.rewrite_text(text_analysis, nil) }.to raise_error(ArgumentError, /Rewrite guidance must be a hash/)
      expect { rewriter.rewrite_text({}, mock_rewrite_guidance) }.to raise_error(ArgumentError, /Text analysis must be a hash/)
    end
  end
  ...
  describe '#generate_pattern_informed_prompts' do
    ...
    it 'handles intonation guidance availability' do
      ...
      intonation_prompts = rewriter.generate_pattern_informed_prompts(text_analysis, mock_rewrite_guidance, 'intonation')
  ...
  describe 'iterative refinement' do
    describe '#apply_iterative_refinement' do
      ...
      it 'applies refinement when needed' do
        ...
        result = rewriter.send(:apply_iterative_refinement, initial_result, mock_rewrite_guidance, { max_iterations: 2 })
      ...
      it 'stops when no refinement needed' do
        ...
        result = rewriter.send(:apply_iterative_refinement, initial_result, mock_rewrite_guidance, { max_iterations: 3 })
      ...
      it 'respects maximum iterations' do
        ...
        result = rewriter.send(:apply_iterative_refinement, initial_result, mock_rewrite_guidance, { max_iterations: 1 })
  ...
  describe 'error handling' do
    it 'handles LLM conversion failures' do
      ...
      expect { rewriter.rewrite_text(text_analysis, mock_rewrite_guidance) }.to raise_error(/LLM rewrite execution failed/)
    end
